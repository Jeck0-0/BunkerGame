...
{
    [Header("Crisis Scheduling")]
    public List<Crisis> CrisisPool = new List<Crisis>();
    ...
    [Tooltip("Starting bunker integrity")]
    public int BunkerIntegrity = 10;
    ...
    private void StartCrisis(Crisis crisis)
    {
        ...
        var dto = new CrisisStartDTO()
        {
            ...
            // usses actual current world time (for networking)
            CrisisEndTimeUtc = DateTime.UtcNow.AddSeconds(crisis.TimeToResolve).Subtract(DateTime.UnixEpoch).TotalSeconds,
        };
    ...
    private void ResolveActiveCrisis()
    {
        ...
        if (globalSuccess)
        {
            ...
            if (!string.IsNullOrEmpty(highestBidder))
            {
                ...
                perPlayerChanges[highestBidder].Add(new ResourceAmount(r.Type, r.Amount));
            }
        }
        else
        {
            ...
            foreach (var player in currentPlayers)
            {
                ...
                perPlayerChanges[player].Add(new ResourceAmount(r.Type, -Mathf.Abs(r.Amount)));
            }
        ...
        foreach (var player in currentPlayers)
        {
            var playerDto = new PlayerResolveDTO
            {
                ...
                PlayerSuccessPoints = perPlayerSuccessPoints.ContainsKey(player) ? perPlayerSuccessPoints[player] : 0,
    ...
    private class ActiveCrisisInstance
    {
        ...
        public Dictionary<string, PlayerContribution> Contributions = new Dictionary<string, PlayerContribution>(StringComparer.Ordinal);
